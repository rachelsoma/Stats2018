...
drives.failed=which(failed=="TRUE" & days < 366) #subset of cases that filed in first year
n.drives.failed=length(drives.failed) #number of cases in subset
paste(length(drives.failed),"drives have failed in the first year")
```
(b) [1 mark] Compute a point estimate for the proportion of drives that fail in the first year of operation.

```{r}
phat=n.drives.failed/n
...
boot.dist = rep(NA, b)
for (i in 1:b) {
  boot.sample = sample(n.drives.failed, replace=TRUE)
  boot.dist[i] = mean(drives.failed[boot.sample])
}
