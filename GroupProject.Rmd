...
attach(drives)
drives.failed = days[days < 366 & failed == "TRUE"]
noFCases = length(drives.failed)
noFCases
noCases = length(days)
noCases
```
...
```{r}
pointEstimate = noFCases / noCases
pointEstimatePercentage = pointEstimate * 100
pointEstimatePercentage
```
...
bootDist = rep(0,b)
for (i in 1:b) {
  s = sample(HardDisks$days, replace = TRUE) #There is something wrong here... can't figure out how to get the sample with two criteria
  bootDist[i] = length(s[1][s[1] < 366 & s[2] == "TRUE"]) / noCases
}

bootDist

quantile(bootDist, c(0.005,0.995))
